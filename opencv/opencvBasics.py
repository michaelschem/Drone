...
# Function: cv2.imread('image_file_path',image_type)
...
# Function: cv2.imshow('window_name',image_file)
cv2.imshow('image',img)
# Function: cv2.waitKey(delay)
# Description: How long the window should stay open
# Parameters: 0 means it will wait until a key is pressed
cv2.waitKey(0)
# closes all windows
...
# Function: cv2.imwrite(output_file_location,image_to_output)
...
# Function: cv2.VideoCapture(source)
# Parameters: 0 means the first available camera that is attached
# Others would follow 1,2,3,...
cap = cv2.VideoCapture(0)
cap.open(0)
# If you want to use a file, just put the file name in instead of a number.
# Just uncomment the next 3 lines and comment 57 and 58 out.
# vid_file = 'videos\\IR_DARK_FILTERED.mp4'
# cap = cv2.VideoCapture(vid_file)
# cap.open(vid_file)

# Leave the capture stream open until you press q
while(cap.isOpened()):
...
Goal is to read an image of IR LEDs and identify the number of LEDs seen
"""
...
# number of circles (won't actually use but good to see)
